...
def calculate_ndcg(keyword: str, retrieved_docs: list, k: int = 10) -> float:
    ...
    dcg = calculate_dcg(relevances=relevances, k=k)
    ideal_relevances = sorted(relevances, reverse=True)
    idcg = calculate_dcg(ideal_relevances, k=k)
...
def evaluate_retrieval(test: TestQuestion, k: int = 10) -> RetrievalEval:
    ...
    mrr_scores = [calculate_mrr(keyword=keyword, retrieved_docs=retrieved_docs) for keyword in test.keywords]
    avg_mrr = sum(mrr_scores) / len(mrr_scores) if mrr_scores else 0.0

    ndcg_scores = [calculate_ndcg(keyword=keyword, retrieved_docs=retrieved_docs, k=k) for keyword in test.keywords]
    avg_ndcg = sum(ndcg_scores) / len(ndcg_scores) if ndcg_scores else 0.0

    keywords_found = sum(1 for score in mrr_scores if score > 0)
    total_keywords = len(test.keywords)
    keyword_coverage = (keywords_found / total_keywords * 100) if keywords_found > 0 else 0.0
...
def evaluate_answer(test: TestQuestion) -> tuple[AnswerEval, str, list]:
    ...
    generated_answer, retrieved_docs = answer_question(question=test.question)
...
Provide detailed feedback and scores from 1 (very poor) to 5 (ideal) for each dimension. If the answer is wrong, then the accuracy score must be 1."""
    ...
    judge_response = completion(messages=judge_messages, model=MODEL, response_format=AnswerEval)

    answer_eval = AnswerEval.model_validate_json(json_data=judge_response.choices[0].message.content)
...
def evaluate_all_answers():
    ...
    for index, test in enumerate(tests):
        result, generated_answer, retrieved_docs = evaluate_answer(test=test)
        progress = (index + 1) / total_tests
...
def run_cli_evaluation(test_row_number: int):
    ...
    retrival_result = evaluate_retrieval(test=test)

    print(f"MRR: {retrival_result.mrr:.4f}")
    print(f"nDCG: {retrival_result.ndcg:.4f}")
    print(f"Keywords Found: {retrival_result.keywords_found}/{retrival_result.total_keywords}")
    print(f"Keyword Coverage: {retrival_result.keyword_coverage:.1f}")
    ...
    answer_result, generated_answer, retrieval_docs = evaluate_answer(test=test)
...
def main():
    ...
    run_cli_evaluation(test_row_number=test_row_number)
